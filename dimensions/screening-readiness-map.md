...
Consolidation status (run-262): all 33 profiles remain depth 10.0, the global verification queue has 0 pending/open items, legacy country-local `unverified_count` values are reconciled, and schema-safe tier application has assigned Greece / Poland Tier 1, Spain / Portugal / Italy / Czech Republic Tier 2, and Cyprus / Croatia / Malta / Romania / Bulgaria Tier X; the remaining 22 countries stay `tier: null`.
...
- Do not treat `tier_hint` as assigned final `tier`; only Greece, Spain, Portugal, Italy, Cyprus, Croatia, Malta, Czech Republic, Poland, Romania, and Bulgaria have assigned final tiers after run-262, while all other countries remain null until explicit rationale is written.
